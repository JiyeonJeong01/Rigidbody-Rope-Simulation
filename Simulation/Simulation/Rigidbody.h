...
public :
	HRESULT			Ready_Component(BODY_DESC* b); // Find~ 함수들 실행하기
...
private :
	...
	void			Calc_Dimension(BODY_DESC* b); // m_vDimension과 m_vDimensionCenter를 구한다
	void			Calc_Inertia(BODY_DESC* b);	
	void			Calc_COM(BODY_DESC* b);
...
public:
	...
	BODY_DESC*			Try_GetMyBody() { return PhysicsWorld::GetInstance()->Try_GetBody(m_iId);}

    const BODY_DESC&     Get_BodyInfo();
...
public:
	static Rigidbody* Create(LPDIRECT3DDEVICE9 pGraphicDev, Object* pOwner, BODY_DESC tBody);
